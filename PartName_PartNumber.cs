    ...
    public partial class PartName_PartNumber : Form
    {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            using (PrintDocument pd = new PrintDocument())
            {
                ...
                // Use the shared handler (with the portrait-bounds safety net below)
                // instead of an inline lambda, so this path and the PDF-export path
                // behave identically.
        ...
        private void PrintLabelsHandler(object sender, PrintPageEventArgs e)
        {
            ...
            {
                // Safety net only: with the Landscape-flag conflict fixed above, this
                // branch normally shouldn't trigger. It's kept in case a given printer
                // driver still reports a portrait-shaped page for this custom size --
                // in that case we rotate our landscape content 90 degrees to fit it
                // correctly instead of letting it render squeezed into a tall canvas.
        ...
        private void DrawTextCentered(Graphics g, string text, string font, FontStyle style, float size, int x, int y, int w, int h, bool isWhite)
        {
            using (Font f = new Font(font, size, style))
            {
                StringFormat sf = new StringFormat { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center };
                g.DrawString(text, f, isWhite ? Brushes.White : Brushes.Black, new RectangleF(x, y, w, h), sf);
            }
        }
        ...
        private void btnPrintAll_Click(object sender, EventArgs e)
        {
            ...
            // Pack 2 parts per page, matching your existing 2-row label layout
            _batchPages.Clear();
